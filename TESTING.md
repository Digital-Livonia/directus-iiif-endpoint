...
Current totals: **46 tests** across 2 files (`npm test`).

---

## Unit layer — `src/helpers.test.js` (38 tests)
...
- `service["@id"]` uses whatever `searchServiceUrl` was passed in (not hardcoded)
- Multiple metadata pairs all appear, in order

---

## Integration layer — `src/handler.test.js` (8 tests)
